...
PnP问题通过已知3D点和2D点的对应关系，来求解相机位姿，针对该问题，有多种求解方法：P3P, 直接线性变换(DLT)，EPnP， UPnP等，还有非线性优化的方法，构建最小二乘并迭代求解，即Bandle Adjustment （BA）。

##### DLT :
##### P3P:
通过将2D点转化到相机坐标系中，然后得到了3D-3D的对应点，把PnP问题转换为ICP问题。然后对其进行求解。
缺点：
- P3P只利用了3个点的信息，当给定的配对点多于3时，难以利用更多的信息
- 如果3D点或者2D点受噪声影响，或者存在误匹配，则算法失效。
##### 最小化重投影误差求解PnP
![[7-3 figure 4.png]]

通过构建Ceres 或者g2o都可以求解。

#### 3D-3D： ICP

ICP问题简单描述为： 已知两组三维点已经他们之间的点对匹配关系$P_1, P_2$， 然后想要找一个欧式变换$R ,t$，使得:
${P_1}_i = R * {P_2}_i + t$
求解ICP问题方法分为两种，一种是通过线性代数SVD求解，另一种是通过非线性优化方式的求解（类似于BA）。
在视觉SLAM中因为有视觉信息，所以可以知道三维的匹配关系，但是在激光SLAM中可能不知道三维的匹配关系，一般认为距离最近的两个点为同一个，这个方法称为迭代最近点。
...
##### BA优化方法：
同样优化一个误差函数：
$\frac{1}{2} \sum ^n_{i = 1} \|(p1_i - exp(p2_i)\|^2_2$



# 第8章 视觉里程计 2

这一讲主要讲了使用光流法去找到二维匹配点对，然后通过这些点去用上一张中的方法进行位姿估计的问题。
- 使用光流法可以减少计算描述子和特征点匹配的时间问题
- 同时不要求由特征点，由于光流法是主要通过梯度来计算的
- 还能构建半稠密甚至稠密地图
但是光流法本身的假设太强，灰度值不变，实际过程中这一点达不到，同时求解过程也存在一些问题。
